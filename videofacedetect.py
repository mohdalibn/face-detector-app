...
trained_faced_data = cv2.CascadeClassifier('Face Detector App/haarcascade_frontalface_default.xml') 


#video = cv2.VideoCapture(0) # this line will use the webcam of the laptop
video = cv2.VideoCapture('./Face Detector App/DeliveryDriver.mp4')
...
while True:
    ...
    new_img = cv2.resize(frame, (1000, 720)) 

    cv2.imshow("Frontal face detector", new_img)
        
    key = cv2.waitKey(1) # '1' represents the time in milisecs this function waits on a single frame
